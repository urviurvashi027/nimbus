...
const ScribbleContext = createContext<ScribbleContextType | undefined>(
  undefined
);

export const ScribbleProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const addScribble = async (newScribble: Omit<Scribble, "id" | "date">) => {
    const saved = await scribbleService.saveScribble({
      title: newScribble.title,
      content: newScribble.content,
      tag_list: newScribble.tags,
    });
  ...
  return (
    <ScribbleContext.Provider
      value={{ scribbles, loading, addScribble, refreshScribbles }}
    >
      {children}
    </ScribbleContext.Provider>
  );
